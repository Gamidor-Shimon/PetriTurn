...
[logs]
max_mb = 5           ; size of one log file; a new file every day and at this size
```
...
| `max_mb` | גודל מקסימלי של קובץ לוג אחד, במגה-בייט (ראו [לוגים](#לוגים)) |
...
| קובץ | מי כותב |
|---|---|
| `2026-09-26_robot_001.log` | `petriturn.exe` — כל קריאה של הרובוט |
| `2026-09-26_gui_001.log` | `petriturn_gui.exe` — כל מה שמופיע בקונסולה של ה-GUI |

- **קובץ חדש כל יום**, ובנוסף כשהקובץ מגיע ל-`max_mb` (ברירת מחדל 5 MB) — ממשיכים ב-`_002`, `_003`...
- **מה נרשם על כל קריאה של הרובוט:** הפקודה המלאה, המשתמש והמחשב, הפורט, **כל שורה שנשלחה לבקר ושהתקבלה ממנו**,
  התוצאה, וקוד היציאה עם משך הזמן.
- כל שורה מתחילה בתאריך ושעה עד אלפית השנייה, ובמספר התהליך (`pid`) — כדי להבדיל בין קריאות.

```text
2026-09-26 14:03:12.340 [pid 4812] [start] petriturn 1.1 | command: petriturn rotate 0.25 10 cw | user lab1 | PC ROBOT-PC
2026-09-26 14:03:12.352 [pid 4812] [port] COM6 opened
2026-09-26 14:03:12.353 [pid 4812] [tx] INFO
2026-09-26 14:03:12.360 [pid 4812] [rx] OK SLOTS=20 MAX_STEPS=32 ...
2026-09-26 14:03:12.361 [pid 4812] [tx] ROT 90 10
2026-09-26 14:03:14.050 [pid 4812] [rx] OK
2026-09-26 14:03:14.051 [pid 4812] [result] OK
2026-09-26 14:03:14.052 [pid 4812] [exit] code 0 after 1.71 s
```

- **אם אי אפשר לכתוב לוג** (אין הרשאה, דיסק מלא): הרובוט מקבל אזהרה `WARN` ב-stderr (הפקודה עצמה מתבצעת),
  וה-GUI מציג בקונסולה באדום `LOG NOT SAVED`.
- **נפח:** קריאה של הרובוט היא כ-0.5 KB. 1,000 קריאות ביום ≈ 0.5 MB ביום ≈ 200 MB בשנה.
  מומלץ לגבות את התיקייה `logs` יחד עם שאר נתוני המעבדה.
...
בתחתית — קונסולה עם כל פקודה ותשובה, שנשמרת גם בתיקיית `logs` (ראו [לוגים](#לוגים)).
